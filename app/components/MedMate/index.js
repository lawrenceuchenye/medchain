...
const index=()=>{
    ...
    return(
        <div className={styles.overlay} onClick={()=>setIsRequestDES(false)}>
            <div className={styles.chatContainer} onClick={(e)=>e.stopPropagation()}>
                <div className={styles.chatsContainer}  ref={scrollContainer} id="chatsContainer">
                    {chats.map((chat)=>{
                ...
                <div className={styles.btn} onClick={async ()=>{
                    if(chat.length > 0){
                        ...
                        setScrollPos(scrollPos+res.length/3);
                        setChat("");
                        chats.push({ content:res,pos:false });
                        scrollContainer.current.scrollTo(0,scrollContainer.current.scrollHeight+scrollPos);
                    }    
                }}>
